...
| `SENTRY_WEBHOOK_SECRET` | Sentry (secret du webhook) | Agent Maintenance (`app/routers/maintenance.py`, webhook entrant) | Vérifie la signature HMAC-SHA256 (`Sentry-Hook-Signature`) des alertes Sentry reçues, pour empêcher l'injection de fausses alertes. Vide par défaut : la vérification est alors ignorée (dégradation explicite, non sécurisée). |
| `TOKEN_ENCRYPTION_KEY` | Interne (pas un service tiers) | Plateforme (`platform_core/encryption.py`) | Clé Fernet chiffrant au repos `Client.meta_page_access_token` et `Client.whatsapp_access_token`. Vide par défaut : stockage en clair (dégradation explicite pour le développement local/les tests) — à renseigner impérativement en production. |
| `PLATFORM_WHATSAPP_PHONE_NUMBER_ID` / `PLATFORM_WHATSAPP_ACCESS_TOKEN` | WhatsApp Business Cloud API | Agent Planning (`agents/planning/scheduled_jobs.py`, rappels de RDV) | Compte WhatsApp Business de la **plateforme elle-même** (le staff), pas d'un client — distinct de `Client.whatsapp_phone_number_id`/`whatsapp_access_token` (compte du client, utilisé par l'agent Prospection pour que CE CLIENT contacte SES PROPRES prospects). Vide par défaut : la tâche planifiée de rappel ne fait rien. |
